...
# Script: Tables and Figures for the Paper
# Update: January 2024
...
#-------------------------------------------------------------------------------
# Compute Tax Changes
taxchanges_data <- tax_data_clean %>% 
...
taxchanges_deltarate <- taxchanges_data %>% 
  ...
  ## sum all tax changes across years 
  group_by(year) %>% 
  summarize(`Average Tax Increase` = 100*mean(delta_tax_inc, na.rm = TRUE), 
            `Average Tax Decrease` = 100*mean(delta_tax_dec, na.rm = TRUE))

taxchanges_table <- taxchanges_rate %>% 
  left_join(taxchanges_deltarate, by = "year", relationship = "one-to-one") 

# Tax Increases and Decreases Expressed in Basis Points 
# Multiply by 100 
...
graph1_counts <- taxchanges_year %>% 
  ...
  scale_x_continuous(breaks = 2002:2023) + scale_y_continuous(n.breaks = 20) +
...
# Table 2 Statistics 
table2 %>% 
  select(State, `Number of Tax Increases`, `Number of Tax Decreases`) %>% 
  summarize(taxincmean = mean(`Number of Tax Increases`, na.rm = TRUE), 
            taxincmedn = median(`Number of Tax Increases`, na.rm = TRUE), 
            taxdecmean = mean(`Number of Tax Decreases`, na.rm = TRUE), 
            taxdecmedn = median(`Number of Tax Decreases`, na.rm = TRUE))

table2 %>% 
  select(State, `Number of Tax Increases`, `Number of Tax Decreases`) %>% 
  mutate(incdummy = ifelse(`Number of Tax Increases` > 0, 1, 0), 
         decdummy = ifelse(`Number of Tax Decreases` > 0, 1, 0)) %>% 
  summarize(`At least one increase` = sum(incdummy), 
            `At least one decrease` = sum(decdummy))


#-------------------------------------------------------------------------------
# Table 3. Number of States by Number of tax Change 
# Data frame with the number of tax changes by state 
table3data <- taxchanges_state %>% 
...
table3 <- table3data %>% 
  ...
  summarize(States = toString(State)) %>% 
...
# Number of States with between and 1 and 4 tax rate changes 
table3data %>%  
  group_by(`Number of Tax Changes`) %>% 
  summarize(n = n()) %>% 
  filter(`Number of Tax Changes` >= 1 & `Number of Tax Changes` <= 4) %>% 
  ungroup() %>% 
  summarize(n = sum(n))

#------------------------------------------------------------------------------
# Comparison of state rates 2002-2023
rate_comparison <- taxchanges_data %>% 
  select(state, year, taxrate) %>% 
  filter(year == 2002 | year == 2023) %>% 
  spread(year, taxrate) %>% 
  mutate(change = case_when(`2023`>`2002` ~ "Larger", 
                            `2023`<`2002` ~ "Smaller", 
                            `2023`==`2002` ~ "Constant")) %>% 
  mutate(delta = 100*(`2023` - `2002`)) %>% 
  arrange(delta)

rate_comparison_number <- rate_comparison %>% 
  group_by(change) %>% 
  summarize(n = n())

rate_comparison_states <- rate_comparison %>% 
  group_by(change) %>% 
  summarize(states = toString(state)) %>% 
  left_join(rate_comparison_number, by = "change", relationship = "one-to-one") %>% 
  relocate(change, n, states)

write_csv(x = rate_comparison_states, 
          file = paste(ao, "rate_comparison_states.csv", sep = ""))
...
taxincreasers_plot <- taxplot(data = tax_data_clean %>% 
                                filter(tax_policy_seg == "Tax Increasers"), 
                              main = "PIT Rate - Tax Increasers")
taxdecreasers_plot <- taxplot(data = tax_data_clean %>% 
                                filter(tax_policy_seg == "Tax Decreasers"), 
                              main = "PIT Rate - Tax Decreasers")
taxswitchersinc_plot <- taxplot(data = tax_data_clean %>% 
                               filter(tax_policy_seg == "Tax Switchers - Net Increaser"), 
                             main = "PIT Rate - Tax Switchers - Net Increaser")
taxswitchersdec_plot <- taxplot(data = tax_data_clean %>% 
                                  filter(tax_policy_seg == "Tax Switchers - Net Decreaser"), 
                                main = "PIT Rate - Tax Switchers - Net Decreaser")
notaxchange_plot <- taxplot(data = tax_data_clean %>% 
                              filter(tax_policy_seg == "No Tax Change"), 
                            main = "PIT Rate - No Tax Change")
nopitrate_plot <- taxplot(data = tax_data_clean %>% 
